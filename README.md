...
[JJYシミュレータWeb版](https://shogo82148.github.io/web-jjy/)ですぐに試せるデモページを公開してます。
...
MacBook Air + Google Chrome + CITIZEN 8RZ152 で動作を確認しています。
...
詳しくは[フリーソフト電波時計用JJYシミュレータのページ](https://www.starstonesoft.com/jjy_simulator.htm)を参照してください。

## 開発

```bash
npm install          # 依存パッケージのインストール
npm run build        # esbuild: src/jjy.ts → jjy.js (IIFEバンドル)
npm run typecheck    # tsc --noEmit (型チェック)
npm test             # vitest run (テスト実行)
npm run run          # esbuild開発サーバー起動 (http://localhost:8000/)
```
